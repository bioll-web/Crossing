...
```mermaid
flowchart TB
    subgraph serial["🐢 串行 Serial"]
        direction LR
        s1["任务 A"] --> s2["任务 B"] --> s3["任务 C"]
    end
 
    subgraph concurrent["🔀 并发 Concurrent（单核）"]
        direction LR
        c1["任务 A"] --> cx1{{"切换"}} --> c2["任务 B"] --> cx2{{"切换"}} --> c3["任务 A"] --> cx3{{"切换"}} --> c4["任务 B"]
    end
 
    subgraph parallel["⚡ 并行 Parallel（多核）"]
        direction TB
        p1["核心1：任务 A ▓▓▓▓▓▓▓▓"]
        p2["核心2：任务 B ▓▓▓▓▓▓▓▓"]
    end
 
    style serial fill:#f1f5f9,stroke:#94a3b8,color:#334155
    style concurrent fill:#ede9fe,stroke:#6366f1,color:#1e1b4b
    style parallel fill:#ddd6fe,stroke:#4f46e5,color:#1e1b4b
```
...
虽然我们知道 CPU 肯定是快的，但是键盘，打印机，磁盘，磁带这样的外设，同时在这样一个进程里参与，他们的时间相比于 CPU 就很慢。举个例子，如果 CPU 要算 1 秒钟，但等到打印机打印结果，从磁盘里读取数据等等操作却需要数十秒。这样的情况就会导致 CPU 在这些无用的时间内要空转 / 发呆，硬件资源就被浪费了。所以后来人们又发明了批处理系统，直到最关键的进步——多道程序设计的诞生，这样的问题才终于得到了一定的解决，并出现了并发性。也就是说，并发性本身是结构性，这种结构性将会引起我们对整个系统乃至功能的设计，所以说它至关重要。
...
flowchart TB
    ...
    style V fill:#6366f1,color:#fff,stroke:#4f46e5
    style T fill:#a5b4fc,color:#1e1b4b,stroke:#6366f1
    style S fill:#a5b4fc,color:#1e1b4b,stroke:#6366f1
```
